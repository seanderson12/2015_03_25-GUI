...
%        setappdata(nk2gui2,'z_safe_um',z_safe_um);
        ...
        set(move2_msg,'Position',[50 70 200 60]);% ʹ������������޸�msgbox��λ�úʹ�С
...
S_inject_um=uicontrol('String', 'Inject(um) Selection','Position', [230 398 100 20], 'Callback', {@Inject_S_um,move_speed_um,inject_speed_um ,zrefer_um,relativeangle,delay_time,MainFigure}) ;
...
%% wave
        Wave_timer= timer('Name','ButtonTimer','StartDelay', 0,'Period', 0.3 ,...
                   'ExecutionMode','fixedRate',...
                   'StartFcn','[hwave,hdevice,Cbox,capacitor]=wave_init();',...
...
S_inject_ms=uicontrol('String', 'Inject(ms) Selection','Position', [340 398 100 20], 'Callback', {@Inject_S_ms,move_speed_ms,inject_speed_ms ,zrefer_ms,relativeangle,delay_time,MainFigure}) ;
    ...
    function Inject_S_ms(hObject,event,move_speed_ms,inject_speed_ms ,zrefer_ms,relativeangle,delay_time,MainFigure)
        ...
        number=1;
        while 1
            [x(number), y(number), inputflag]=ginputc(1);
            if inputflag ~= 1
                break
            end
            hold on 
        ...
        % solved TPS by 'Copyright (c) 2007, Joseph Kirk'
        [optRoute,minDist] = tsp_ga(xy,dmat,popSize,numIter,0,0);
        %[optRoute,minDist] = tsp_ga(xy,dmat,popSize,numIter,showProg,showResult);
