...
Calabration=uicontrol('String', 'Calabration','Position', [140 398 80 20], 'Callback', {@ui_calabration,move_speed_ms }) ;

    function ui_calabration(hObject,event,move_speed_ms )
        ...
        set(move1_msg,'Position',[100 150 180 60]);% 使用这个语句可以修改msgbox的位置和大小
        uiwait(move1_msg);
...
%        setappdata(nk2gui2,'z_safe_um',z_safe_um);
        ...
        set(move2_msg,'Position',[50 70 200 60]);% 使用这个语句可以修改msgbox的位置和大小
        uiwait(move2_msg);
